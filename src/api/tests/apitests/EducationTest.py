...
class EducationTest(CreateAdminMixin, CreateUserMixin, APITestCase):
    """Unit-tests for education-list and attach_offer_education-list."""
    ...
    def setUp(self):
        ...
        self.education = self.user.applicant.educations.first()

        self.list_url = reverse('education-list')
        self.detail_url = reverse('education-detail', args=[self.education.pk])
    ...
    def test_list_accept_logged_in(self, is_admin_user, rejected):
        """Test that the education list shows up as expected."""
        # Login
        if is_admin_user:
            self.admin_login()
        else:
    ...
    def test_detail_accept_logged_in(self, is_admin_user, rejected):
        ...
        if rejected:
            ...
            self.assertEqual(response.status_code, 200)
            self.assertEqual(
                response.data['edu_inst'], self.education.edu_inst)
            self.assertEqual(response.data['name'], self.education.name)
