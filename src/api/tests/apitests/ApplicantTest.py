...
class ApplicantTest(CreateAdminMixin, CreateUserMixin, APITestCase):
    """Unit-tests for applicant-list and attach_offer_applicant-list."""
    ...
    def setUp(self):
        ...
        self.list_url = reverse('applicant-list')
        self.detail_url = reverse('applicant-detail', args=[self.applicant.pk])
    ...
    def test_list_accept_logged_in(self, is_admin_user, rejected):
        """Test that the applicant list shows up as expected."""
        # Login
        if is_admin_user:
            self.admin_login()
        else:
    ...
    def test_detail_accept_logged_in(self, is_admin_user, rejected):
        ...
        if rejected:
            ...
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.data['pk'], self.applicant.pk)
            self.assertEqual(response.data['work_income'],
                             self.applicant.work_income)
