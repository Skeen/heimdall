...
def check_javascript():
    ...
    import json
    package_json_admin = json.load(open("adminapp/frontend/package.json", 'r'))
    package_json_web = json.load(open("webapp/frontend/package.json", 'r'))

    for package_name, package_version in package_json_admin['dependencies'].items():
        req_packages['common'][package_name] = {}
        req_packages['common'][package_name] = package_version

    for package_name, package_version in package_json_web['dependencies'].items():
        req_packages['common'][package_name] = {}
        req_packages['common'][package_name] = package_version

    for package_name, package_version in package_json_admin['devDependencies'].items():
        req_packages['dev'][package_name] = {}
        req_packages['dev'][package_name] = package_version

    for package_name, package_version in package_json_web['devDependencies'].items():
        req_packages['dev'][package_name] = {}
        req_packages['dev'][package_name] = package_version

    # Load in the licence packages
    import ast
    for filename in ['common', 'dev']:
        lic_packages = {}
        with open("licence/mapping/javascript/" + filename + ".py", 'r') as file_in:
            licence_map = ast.literal_eval(file_in.read())
            for package_name, dicty in licence_map.items():
                dicty.pop('licence', None)
                dicty.pop('homepage', None)
        ...
        # Find the symmetric set difference between the packages
        set_symmetric_difference = (set(lic_packages.items()) ^
                                    set(req_packages[filename].items()))

        # Print differences
        for key in dict(set_symmetric_difference).keys():
            print "Javascript: " + key + " differs in " + filename
